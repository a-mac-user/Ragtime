...
class Asset(object):
    ...
    def __update_asset_component(self, data_source, fk, update_fields, identify_field=None):
        """
        :param data_source:要更新的部件的汇报过来的数据
        :param fk:
        :param update_fields:要更新的字段
        :param identify_field:识别字段，如果none就用资产id识别
        :return:
        ...
        identify_field: use this field to identify each component of an Asset , if set to None,means only use asset id
        to identify
         '''
        """
                        ...
                        for source_data_item in data_source:  # 到客户端的数据 源里去找到跟 服务器数据库中key_field_data对应的条目
                            ...
                            if key_field_data_from_source_data:
                                if key_field_data == key_field_data_from_source_data:
                                    # find the matched source data for this component,
                                    # then should compare each field in this component to see if
                                    # there's any changes since last update
                            ...
                            print('\033[33;1mError:cannot find any matches in source data by using key '
                                  'field val [%s],component data is missing in reporting data!\033[0m' %
                                  (key_field_data, ))
                            self.response_msg("error", "AssetUpdateWarning",
                                              "Cannot find any matches in source data by using key field val "
                                              "[%s],component data is missing in reporting data!" %
                                              (key_field_data,))
                        ...
                        for key, source_data_item in data_source.items():
                            ...
                            if key_field_data_from_source_data:
                                if key_field_data == key_field_data_from_source_data:
                                    ...
                                    # each field in this component to see if there's any changes since last update
                                    self.__compare_component(model_obj=obj, fields_from_db=update_fields,
                                                             data_source=source_data_item)
                                    break  # must break ast last ,then if the loop is finished ,
                                    ...
                                    # this item is lacked from source data, it makes sense when the hardware
                                    #  info got changed. e.g: one of the RAM is broken, sb takes it away,
                                    # then this data will not be reported in reporting data
                            else:  # key field data from source data cannot be none
                                self.response_msg('warning',
                                                  'AssetUpdateWarning',
                                                  "Asset component [%s]'s key field [%s] is not provided in "
                                                  "reporting data " % (fk, identify_field))

                        else:  # couldn't find any matches, the asset component must be broken or changed manually
                            print('\033[33;1mWarning:cannot find any matches in source data by using key '
                                  'field val [%s],component data is missing in reporting data!\033[0m' %
                                  (key_field_data,))
                    else:
            ...
            for k, data in all_components.items():
                ...
                if data.get(identify_field):
                    ...
                    if k in add_list:
                        data[
                            identify_field] = k
                        # add this key into dict , because this dict will be used
                        #  to create new component item in DB
                        will_be_creating_list.append(data)
