...
def render_page_num(request, paginator_obj, loop_counter):
    ...
    if loop_counter == paginator_obj.number:  # current page
        return mark_safe('''<li class='active'><a href="{abs_url}">{page_num}</a></li>'''.format(abs_url=url,
    ...
    if abs(loop_counter - paginator_obj.number) < 2 or \
        ...
        return mark_safe('''<li><a href="{abs_url}">{page_num}</a></li>'''.format(abs_url=url,
                                                                                  page_num=loop_counter))
    elif abs(loop_counter - paginator_obj.number) < 3:
        return mark_safe('''<li><a href="{abs_url}">...</a></li>'''.format(abs_url=url,
                                                                           page_num=loop_counter))
    else:
        return ''
